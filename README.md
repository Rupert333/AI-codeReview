...
本工具通过 Git Hook 自动调用大语言模型（支持智谱GLM和OpenAI）进行代码评审，并将评审结果保存为 Markdown 格式的文件。脚本支持在代码提交时自动运行，并为开发团队提供即时反馈。

## 功能特点

- **自动化代码评审**：在代码提交时，脚本自动调用大语言模型，进行全面的代码评审。
- **多模型支持**：支持智谱GLM和OpenAI模型，可通过配置文件灵活切换。
...
2. **启用调试模式**：
   ...
   如需查看详细的请求和响应信息，可在 `config.sh` 中将 `debug_mode` 设置为 `true`：
